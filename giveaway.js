...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (owner.get(`owners.${message.author.id}`) || message.member.roles.cache.has(pga.fetch(`permga_${message.guild.id}`)) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            if (!giveawayChannel) {
                return message.reply(":x: Vous devez sp̩cifi̩ un salon valide !");
            }
            ...
            if (!giveawayDuration || isNaN(ms(giveawayDuration))) {
                return message.reply(":x: Vous devez sp̩cifier un dur̩e valide!");
            }
            ...
            if (isNaN(giveawayNumberWinners) || parseInt(giveawayNumberWinners) <= 0) {
                return message.reply(
                    ":x: Vous devez sp̩cifier le nombre de gagnants !"
                );
            }
            ...
            if (!giveawayPrize) {
                return message.reply(":x: Vous devez sp̩cifier un gain valide !");
            }
            ...
            message.reply(`Giveaway lanc̩ ${giveawayChannel}!`);
        }
    }
}
