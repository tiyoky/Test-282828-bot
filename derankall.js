    ...
    async execute(client, message, args) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            const embed = new Discord.MessageEmbed()
                .setColor(color)
                .setTitle("Voici les personnes qui ont Ã©tÃ© derank")
            ...
            const alert = new Discord.MessageEmbed()
                .setColor(color)
                .setTitle(`${message.author.tag} Ã  effectuÃ© un derank all`)
                .setDescription(`<a:Abus:957097615147810956> Toutes les personnes possÃ©dant des permissions **Dangereuses** ont Ã©taient derank\n Executeur : <@${message.author.id}>`)
                .setTimestamp()
                .setFooter({ text: `â ï¸ ${footer}` })
